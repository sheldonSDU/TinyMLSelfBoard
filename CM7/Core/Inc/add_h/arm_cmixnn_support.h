 ...
 */
  ...
  *out1 = __UXTB16(__ROR(inA, 12) & 0x000F000F);
  ...
  //预先排列权重
  __STATIC_INLINE void *read_and_pad_ordered_u4(void *source, int16_t *out1, int16_t *out2, int16_t *out3, int16_t *out4)
  {

	int32_t inA = *__SIMD32(source)++;
#ifndef ARM_MATH_BIG_ENDIAN
	*out1 = __UXTB16(inA & 0xF00F);
	*out2 = __UXTB16(__ROR(inA, 4) & 0xF00F);
	*out3 = __UXTB16(__ROR(inA, 8) & 0xF00F);
	*out4 = __UXTB16(__ROR(inA, 12) & 0xF00F);
#else
  *out4 = __UXTB16(inA & 0x000F000F);
  *out3 = __UXTB16(__ROR(inA, 4) & 0x000F000F);
  *out2 = __UXTB16(__ROR(inA, 8) & 0x000F000F);
  *out1 = __UXTB16(__ROR(inA, 12) & 0x000F000F);
  ...
  //部分结果后处理函数
//  __STATIC_INLINE  void result_reordered_int4(int32_t *input, int32_t *output)
//  {
//  	int32_t processed_input = __SXTB16((*input & 0x000FF000) << 4 | (*input & 0xFF));
//  	*output = __SADD16(*output ,processed_input);
//  }
  __STATIC_INLINE  void result_reordered_int4(int32_t *input, int32_t *output1, int32_t *output2)
   {
   	int8_t processed_input1 = (int8_t)(*input & 0xFF);
   	int8_t processed_input2 = (int8_t)((*input >> 12) & 0xFF);
   	*output1 += processed_input1;
   	*output2 += processed_input2;
   }

//  __STATIC_INLINE  void result_reordered_uint4(int32_t *input, int32_t *output1, int32_t *output2)
//   {
//   	int16_t processed_input1 = (int16_t)(*input & 0x3FF);
//   	int16_t processed_input2 = (int16_t)((*input >> 10) & 0x3FF);
//   	*output1 += processed_input1;
//   	*output2 += processed_input2;
////    *output1 += (int16_t)(*input & 0x3FF);
////    *output2 += (int16_t)((*input >> 10) & 0x3FF);
//   }
  __STATIC_INLINE void result_reordered_uint4(int32_t *sum, int32_t *sum2, int32_t *res1, int32_t *res3)
  {
	  int32_t val1 = *sum  & 0x3FF;
	  int32_t val2 = *sum2 & 0x3FF;
      *res1 += val1;
      *res3 += val2;
  ...
  __STATIC_INLINE  void result_reordered_int2(int32_t *input, int32_t *output1, int32_t *output2)
   {
   	int8_t processed_input1 = (int8_t)(*input & 0xFF);
   	int8_t processed_input2 = (int8_t)((*input >> 12) & 0xFF);
   	*output1 += processed_input1;
  ...
  *out1 = __UXTB16(__ROR(inA, 14) & 0x00030003);
  ...
  //2bit
  __STATIC_INLINE void *read_and_pad_ordered_u2(void *source, int32_t *out1, int32_t *out2, int32_t *out3, int32_t *out4,
		  int32_t *out5,int32_t *out6,int32_t *out7,int32_t *out8)
   {
 	    int32_t* src_ptr = (int32_t*)source;
 	    *out1 = *src_ptr++;
 	    *out2 = *src_ptr++;
 	    *out3 = *src_ptr++;
 	    *out4 = *src_ptr++;
  ...
  __STATIC_INLINE int32_t __HI_SMULL(int32_t a, int32_t b)
  {
    int hi = 0;
    int lo = 0;
    __asm volatile("SMULL %[lo_out], %[hi_out], %[a_operand], %[b_operand]"

...
__STATIC_FORCEINLINE int16_t arm_nn_read_s8_to_s16_ia(int8_t **in_s8)
{
    int16_t val = (int16_t)(**in_s8);  // 读取当前指针指向的 8 位数并转换为 16 位数
    (*in_s8)++;  // 指针递增，指向下一个 8 位数
    return val;  // 返回转换后的 16 位值
}
 ...
 *            2,          s4_x
 */
__STATIC_FORCEINLINE uint32_t transform(uint32_t input) {
    uint32_t result = 0;

    result |= (input & 0x0000000F);         // 提取0：3
    result |= (input & 0x000000F0) << 4;   // 提取4：7
    result |= (input & 0x00000F00) << 8;   // 提取8：11
    result |= (input & 0x0000F000) << 12;  // 提取 12：15
...
__STATIC_FORCEINLINE void read_and_pad_s4_ordered(const int8_t *source, int32_t *out1, int32_t *out2)
{
    int16_t in = arm_nn_read_s8x2(source);
    int32_t inA = (in & 0x00FF) | ((in & 0xFF00) << 8);
    int32_t inAbuf1 = SXTB16_RORn(SXTB16(inA), 4);	//__sxtb16
    int32_t inAbuf2 = SXTB16_RORn(SXTB16(inA << 4), 4); //__sxtb16
...
//3bit权重打包
__STATIC_FORCEINLINE void read_and_pad_s3_ordered(const int8_t *source, int32_t *out1, int32_t *out2)
{
	int16_t in_1 = arm_nn_read_s8x2_ia(&source);
	int16_t in_2 = arm_nn_read_s8x2_ia(&source);
	*out1 = ((in_1 >> 8)& 0xFF) << 16 | (in_1 & 0xFF);
	*out2 = ((in_2 >> 8)& 0xFF) << 16 | (in_2 & 0xFF);
}


/*by me*/
__STATIC_FORCEINLINE void read_and_pad_s4_ordered2(const int8_t *source, int32_t *out1, int32_t *out2)
{
    int16_t in = arm_nn_read_s8x2(source);
    int32_t inA = (in & 0x00FF) | ((in & 0xFF00) << 8);
    int32_t inAbuf1 = SXTB16_RORn(SXTB16(inA), 4) & 0x00FF00FF;	//__sxtb16
    int32_t inAbuf2 = SXTB16_RORn(SXTB16(inA << 4), 4) & 0x00FF00FF; //__sxtb16
    #ifndef ARM_MATH_BIG_ENDIAN
    *out2 = (int32_t)(PKHTB(inAbuf1, inAbuf2, 16)) << 8;
    *out1 = (int32_t)(PKHBT(inAbuf2, inAbuf1, 16)) << 8;
    #else
    *out1 = (int32_t)(PKHTB(inAbuf1, inAbuf2, 16));
    *out2 = (int32_t)(PKHBT(inAbuf2, inAbuf1, 16));
    #endif
...
__STATIC_FORCEINLINE void read_and_pad_s4_input_ordered(int32_t *input_1, int32_t *input_2, int32_t *out)
{
//    int32_t inA = ((*input_1 & 0xF) | (((*input_1 >> 4) & 0xF) << 16));
//    int32_t inB = (((*input_2 & 0xF) << 8) | (((*input_2 >> 4) & 0xF) << 24));
//    *out = inA + inB;
	  int32_t inA = ((*input_1 & 0x00FF00) << 8) | (*input_1 & 0x000000FF);
	  int32_t inB = ((*input_2 & 0xF)<<12) | (((*input_2>>8)&0x0F)<<28);
	  *out = inA +inB;


}

__STATIC_FORCEINLINE void read_and_pad_s3_input_ordered(int32_t *input_1, int32_t *input_2, int32_t *out)
{
//    int32_t inA = ((*input_1 & 0xF) | (((*input_1 >> 4) & 0xF) << 16));
//    int32_t inB = (((*input_2 & 0xF) << 8) | (((*input_2 >> 4) & 0xF) << 24));
//    *out = inA + inB;
	  int32_t inA = ((*input_1 & 0x003F00) << 8) | (*input_1 & 0x0000003F);
	  int32_t inB = ((*input_2 & 0x07)<<13) | (((*input_2>>8)&0x07)<<29);
	  *out = inA +inB;
}


__STATIC_FORCEINLINE void read_and_pad_s4_weight_ordered(const int8_t *source, int32_t *out1, int32_t *out2,int8_t *i)
{
    int16_t in = arm_nn_read_s8x2(source);
    int32_t in_A = (in & 0x00FF);
    int32_t in_B = ((in >> 8) & 0x00FF);
    *out1 = ((in_A & 0xF) << 12) | (((in_A >> 4) & 0xF) << 28);
//    if(*out1 & 0x80000000)
//    {
//    	*i = *i+1;
//    }
    *out2 = ((in_B & 0xF) << 12) | (((in_B >> 4) & 0xF) << 28);
//    if(*out2 & 0x80000000)
//    {
//    	*i = *i+1;
//    }
...
int8_t *arm_nn_mat_mult_kernel_s4_s162(const int8_t *input_a,
...
__STATIC_FORCEINLINE int32_t arm_nn_divide_by_power_of_two(const int32_t dividend, const int32_t exponent)
{
    ...
    int32_t remainder = remainder_mask & dividend; //得到 dividend除2^exponent的余数

    // Basic division
    result = dividend >> exponent; //除2^exponent

    // Adjust 'result' for rounding (mid point away from zero)四舍五入，结果远离0
...
__STATIC_FORCEINLINE int32_t arm_nn_requantize(const int32_t val, const int32_t multiplier, const int32_t shift)
{
    ...
    return arm_nn_divide_by_power_of_two(arm_nn_doubling_high_mult_no_sat(val * (1 << LEFT_SHIFT(shift)), multiplier),
                                         RIGHT_SHIFT(shift)); //val1/2^val2,四舍五入
#endif
}

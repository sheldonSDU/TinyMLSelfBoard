 ...
 */
   ...
   */
arm_status
arm_convolve_HWC_int4_u4_int4(const uint8_t *Im_in,
					...
					)
{
    ...
    for (i_out_y = 0; i_out_y < top_padding; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < dim_im_out; i_out_x++)
        {
            ...
            {
                pOut = arm_nn_mat_mult_kernel_reordered_int4_int16_int4(wt,
                                                bufferA,
                                                ch_im_out,
    ...
    for (; i_out_y < dim_im_out - bottom_padding; i_out_y++)
    {
        ...
        /* mid part  for (i_out_y=0; i_out_y < dim_im_out; i_out_y++)*/
        for (; i_out_x < dim_im_out - right_padding; i_out_x++)//输出特征图0,0 0,1
        {
            /* This part implements the im2col function 通过这个循环提取1个卷积核大小的输入数据*/
            for (i_ker_y = i_out_y * stride - top_padding; i_ker_y < i_out_y * stride - top_padding + dim_kernel; i_ker_y++)
            {
                arm_int4_to_int16_reordered(
            ...
            if (pBuffer == bufferA + 2 * ch_im_in * dim_kernel * dim_kernel) //2个滤波器大小的数据
            {
                pOut = arm_nn_mat_mult_kernel_reordered_int4_int16_int4(wt,
                                                bufferA,
                                                ch_im_out,

...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
  ...
  static uint8_t rx_size; //待处理数据长?
  
	if(huart->Instance == USART1){
	
		//HAL_UART_Transmit(&huart1,dbg_rx_tmp_buf,Size,0xff);
		HAL_UARTEx_ReceiveToIdle_IT(&huart1,dbg_rx_tmp_buf,DBG_TMP_RXBUFF_SIZE);
		memcpy(dbg_rx_final_buf,dbg_rx_tmp_buf,Size);
		dbg_rx_size = Size;
	...
	if(huart->Instance == UART4){
		...
		HAL_UART_Transmit(&huart1,wifi_rx_tmp_buf,Size,0xff);
		HAL_UARTEx_ReceiveToIdle_IT(&huart4,wifi_rx_tmp_buf,WIFI_TMP_RXBUFF_SIZE);
		memcpy(wifi_rx_final_buf,wifi_rx_tmp_buf,Size);
	}
...
//		{
//      dbg_rx_final_buf[final_index++] =dbg_rx_tmp_buf[(rx_buf_head + i) % DBG_TMP_RXBUFF_SIZE]; // 环形缓冲处理
//      if (final_index >= DBG_FINAL_RXBUFF_SIZE) final_index = 0; // 避免 RxFinal 溢出
//		}
//		
//		/* 完成接收数据的拷贝，更新新接收数据的 */
//		rx_buf_head = rx_buf_head + rx_size;

//		if (rx_buf_head >= DBG_TMP_RXBUFF_SIZE) rx_buf_head = 0;
//		HAL_UARTEx_ReceiveToIdle_DMA(&huart1, dbg_rx_tmp_buf, DBG_TMP_RXBUFF_SIZE);
//	}
}
/* USER CODE END 1 */
